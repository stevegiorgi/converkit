<template>
  <v-container fluid class="header__container">
    <loading
      :active.sync="isLoading"
      color="#fff"
      background-color="#000"
      :can-cancel="true"
      :is-full-page="true"
    ></loading>

    <!-- <div v-for="group in multipleGroupUsers" :key="group.id"> -->
    <v-data-table
      :headers="mainHeaders"
      :items="multipleGroupUsers.flat()"
      item-key="display_name"
      :items-per-page="10"
      :single-expand="singleExpand"
      :sort-by.sync="sortByGroup"
      :sort-desc.sync="sortDescGroup"
    >
      <template v-slot:item="{ item, isExpanded, expand }">
        <tr
          class="data__row"
          color="primary"
          :key="item.id"
          :class="item.id === selectedRow ? 'custom-highlight-row' : ''"
          @click="
            selectUser(item.id),
              expand(!isExpanded),
              rowSelect(item.id),
              openUserStats(item.email_address)
          "
        >
          <td class="data__column avatar d-block d-sm-table-cell">
            <v-avatar color="primary" size="48">
              <img :src="`${item.avatar}`" />
            </v-avatar>
          </td>
          <td class="data__column name d-block d-sm-table-cell">
            <span class="user__name">{{
              item.display_name | upperCaseNames
            }}</span>
          </td>
          <td class="data__column email d-block d-sm-table-cell">
            {{ item.email_address }}
          </td>
          <td class="data__column group d-block d-sm-table-cell">
            <span v-for="group in item.groups" :key="group">
              <span v-for="g in selectedGroups" :key="g.id">
                <template v-if="group === g.id">
                  {{ g.name }}
                </template>
              </span>
            </span>
          </td>
          <td
            class="data__column latest-activity d-block d-sm-table-cell"
            v-html="item.latest_activity"
          ></td>
          <td class="data__column lessons-completed d-block d-sm-table-cell">
            {{ item.lessons_completed }}
          </td>
          <td class="data__column last-login d-block d-sm-table-cell">
            {{ item.last_login }}
          </td>

          <!-- <td class="data__column last-login d-block d-sm-table-cell">
            <v-btn color="primary" class="ma-2 zoom-statistics" dark openUserStats(item.email_address)>
              <span class="modal-button">Open Statistics</span>
            </v-btn>
            <v-dialog
              v-model="zoomModal"
              dark
              overlay-opacity="0.1"
              scrollable
              max-width="1200"
            >
              <v-card tile>
                <v-toolbar flat dark color="primary">
                  <v-btn icon dark @click="zoomModal = false">
                    <v-icon>mdi-close</v-icon>
                  </v-btn>
                  <v-toolbar-title>Zoom Participation</v-toolbar-title>
                  <v-spacer></v-spacer>
                  <v-menu bottom right offset-y> </v-menu>
                </v-toolbar>
                <v-card-text>
                  <v-list three-line subheader>
                    <v-list-item>
                      <v-list-item-content>
                        <v-list-item-title
                          >Zoom Meeting Participation</v-list-item-title
                        >

                        <v-data-table
                          :headers="zoomHeaders"
                          :items="participations"
                          :items-per-page="10"
                          item-key="id"
                        >
                          <template slot="items" slot-scope="props">
                            <tr>
                              <td>
                                <v-checkbox
                                  v-model="props.selected"
                                  hide-details
                                ></v-checkbox>
                              </td>
                              <td>Meeting Name</td>
                              <td>Attendance</td>
                            </tr>
                          </template>
                        </v-data-table>
                      </v-list-item-content>
                    </v-list-item>
                  </v-list>
                </v-card-text>
              </v-card>
            </v-dialog>
          </td> -->
        </tr>
      </template>

      <template v-slot:expanded-item="{ headers }">
        <tr>
          <td
            class="data__column user-details__container"
            :colspan="headers.length"
          >
            <h3 class="section__heading">Course Activity</h3>

            <div class="user-activity__inner-container restrict-height">
              <v-container fluid class="user-details__panel">
                <v-row>
                  <v-col cols="12" md="12" class="px-0">
                    <v-data-table
                      :headers="courseActivity"
                      :items="userActivity.course_activities"
                      item-key="id"
                      :hide-default-footer="true"
                      class="user-details__table"
                      :sort-by.sync="sortByCourse"
                      :sort-desc.sync="sortDescCourse"
                    >
                      <template v-slot:item="{ item }">
                        <tr>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.course_title"
                          ></td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_started"
                          >
                            {{ item.activity_started }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_completed"
                          >
                            {{ item.activity_completed }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.activity_duration | duration }}
                          </td>
                        </tr>
                      </template>
                    </v-data-table>
                  </v-col>
                </v-row>

                <v-row>
                  <v-col cols="12" md="12" class="px-0">
                    <v-data-table
                      :headers="lessonActivity"
                      :items="userActivity.lesson_activities"
                      item-key="id"
                      :hide-default-footer="true"
                      class="user-details__table"
                      :sort-by.sync="sortByCourse"
                      :sort-desc.sync="sortDescCourse"
                    >
                      <template v-slot:item="{ item }">
                        <tr>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.activity_title"
                          ></td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.course_title"
                          ></td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_started"
                          >
                            {{ item.activity_started }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_completed"
                          >
                            {{ item.activity_completed }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.activity_duration | duration }}
                          </td>
                        </tr>
                      </template>
                    </v-data-table>
                  </v-col>
                </v-row>

                <v-row v-if="userActivity.topic_activities">
                  <v-col cols="12" md="12" class="px-0">
                    <v-data-table
                      :headers="topicActivity"
                      :items="userActivity.topic_activities"
                      item-key="id"
                      :hide-default-footer="true"
                      class="user-details__table"
                      :sort-by.sync="sortByCourse"
                      :sort-desc.sync="sortDescCourse"
                    >
                      <template v-slot:item="{ item }">
                        <tr>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.activity_title"
                          ></td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.course_title"
                          ></td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_started"
                          >
                            {{ item.activity_started }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_completed"
                          >
                            {{ item.activity_completed }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.actvitiy_duration | duration }}
                          </td>
                        </tr>
                      </template>
                    </v-data-table>
                  </v-col>
                </v-row>

                <v-row>
                  <v-col cols="12" md="12" class="px-0">
                    <v-data-table
                      :headers="quizActivity"
                      :items="userActivity.quiz_activities"
                      item-key="id"
                      :hide-default-footer="true"
                      class="user-details__table"
                      :sort-by.sync="sortByCourse"
                      :sort-desc.sync="sortDescCourse"
                    >
                      <template v-slot:item="{ item }">
                        <tr>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.activity_title"
                          ></td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.course_title"
                          ></td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_started"
                          >
                            {{ item.activity_started }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_completed"
                          >
                            {{ item.activity_completed }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.activity_duration | duration }}
                          </td>
                        </tr>
                      </template>
                    </v-data-table>
                  </v-col>
                </v-row>
              </v-container>
            </div>

            <h3 class="section__heading">Course Progress</h3>

            <div class="user-details__inner-container restrict-height">
              <v-container fluid class="user-details__panel">
                <v-row>
                  <v-col cols="12" md="12" class="px-0">
                    <v-data-table
                      :headers="courseHeaders"
                      :items="filteredCourses"
                      item-key="id"
                      :hide-default-footer="true"
                      class="user-details__table"
                      :sort-by.sync="sortByCourse"
                      :sort-desc.sync="sortDescCourse"
                    >
                      <template v-slot:item="{ item }">
                        <!-- <template v-for="course in userCourses"> -->
                        <!-- <template v-if="item.course === course.id"> -->
                        <tr>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.course_title"
                          ></td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.course_status | status }}
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            <v-chip
                              :color="
                                getColor(
                                  completionPercentage(
                                    item.course_progress.completed,
                                    item.course_progress.total
                                  )
                                )
                              "
                              dark
                            >
                              {{
                                completionPercentage(
                                  item.course_progress.completed,
                                  item.course_progress.total
                                ) + "%"
                              }}
                            </v-chip>
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.date_started"
                          >
                            <!-- {{ item.date_started | moment("calendar") }} -->
                            {{ item.date_started }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.date_completed"
                          >
                            {{ item.date_completed }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                        </tr>
                        <!-- </template> -->
                        <!-- </template> -->

                        <!-- <Lessons :courseId="item.id" /> -->
                      </template>
                    </v-data-table>
                  </v-col>
                  <!-- <v-col
                    cols="12"
                    md="4"
                    class="px-0 progress-chart__container"
                  >
                    <h4 class="progress-chart__title">Overall Progress</h4>
                    <vue-apex-charts
                      type="radialBar"
                      :options="chartOptions"
                      :series="series"
                    >
                    </vue-apex-charts>
                  </v-col> -->
                </v-row>
              </v-container>

              <v-container fluid class="user-details__panel">
                <v-row>
                  <v-col cols="12" class="px-0">
                    <!-- Start Quizzes -->

                    <v-data-table
                      :headers="quizHeaders"
                      :items="filteredQuizzes"
                      item-key="id"
                      :hide-default-footer="true"
                      class="user-details__table"
                    >
                      <template v-slot:item="{ item }">
                        <!-- <template v-for="course in userCourses"> -->
                        <!-- <template v-if="item.course === course.id"> -->
                        <tr>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.quiz_title"
                          ></td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.pass | passed }}
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            <v-chip :color="getColor(item.quiz_score)" dark>
                              {{ item.quiz_score + "%" }}
                            </v-chip>
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.timespent }}
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.quiz_started }}
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.quiz_completed }}
                          </td>
                        </tr>
                        <!-- </template> -->
                        <!-- </template> -->
                      </template>
                    </v-data-table>

                    <!-- End Quizzes -->
                  </v-col>
                </v-row>
              </v-container>
            </div>

            <h3 class="section__heading">Zoom Activity</h3>

            <div class="user-activity__inner-container restrict-height">
              <v-container fluid class="user-details__panel">
                <v-row>
                  <v-col cols="12" md="12" class="px-0">
                    <v-data-table
                      :headers="zoomHeaders"
                      :items="participations"
                      :items-per-page="100"
                      :hide-default-footer="true"
                      item-key="id"
                      class="user-details__table"
                    >
                      <!-- <template v-slot:item="{ item }">
                        <tr>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-html="item.course_title"
                          ></td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_started"
                          >
                            {{ item.activity_started }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-if="item.activity_completed"
                          >
                            {{ item.activity_completed }}
                          </td>
                          <td
                            class="data__column d-block d-sm-table-cell"
                            v-else
                          >
                            ––
                          </td>
                          <td class="data__column d-block d-sm-table-cell">
                            {{ item.activity_duration | duration }}
                          </td>
                        </tr>
                      </template> -->
                    </v-data-table>
                  </v-col>
                </v-row>
              </v-container>
            </div>
          </td>
        </tr>
      </template>
    </v-data-table>
    <!-- </div> -->
  </v-container>
</template>

<script>
import moment from "moment";
import { mapGetters } from "vuex";
import Loading from "vue-loading-overlay";
import "vue-loading-overlay/dist/vue-loading.css";
import { Bus } from "@/frontend/mixins/bus.js";
import VueApexCharts from "vue-apexcharts";

// import ZoomStatistics from "@/frontend/components/ZoomStatistics.vue";

// import UserActivity from "@/frontend/components/UserActivity.vue";
// import vueCustomScrollbar from 'vue-custom-scrollbar'
// import UserDetails from "@/frontend/components/UserDetails.vue";
// import Lessons from "@/frontend/components/Lessons.vue";

export default {
  name: "GroupList",
  components: {
    Loading,
    moment,
    VueApexCharts,
    // UserActivity,
    // UserDetails,
    // Lessons,
  },
  data: () => ({
    series: [78, 67],
    chartOptions: {
      chart: {
        foreColor: "fff",
        type: "radialBar",
        height: 300,
        offsetX: 0,
      },
      // colors: ["#ff9800", "#4caf50", "#fff"],
      colors: ["#ff9800", "#4caf50"],
      plotOptions: {
        radialBar: {
          inverseOrder: false,
          hollow: {
            margin: 5,
            size: "48%",
            background: "transparent",
          },
          dataLabels: {
            showOn: "always",
            name: {
              offsetY: -15,
              fontSize: "18px",
              color: "#fff",
            },
            value: {
              offsetY: -0,
              fontSize: "30px",
              fontWeight: 700,
              show: true,
              color: "#fff",
            },
          },
          track: {
            show: true,
            background: "#fff",
            strokeWidth: "98%",
            opacity: 0.3,
            margin: 3, // margin is in pixels
          },
        },
      },
      labels: ["Courses", "Lessons"],
      stroke: {
        lineCap: "round",
      },
      legend: {
        show: true,
        position: "right",
        offsetX: -20,
        offsetY: 0,
        formatter: function (val, opts) {
          return val + " - " + opts.w.globals.series[opts.seriesIndex] + "%";
        },
      },
      // fill: {
      //   type: "gradient",
      //   gradient: {
      //     shade: "dark",
      //     type: "radial",
      //     shadeIntensity: 0.1,
      //     inverseColors: false,
      //     opacityFrom: 1,
      //     opacityTo: 1,
      //     stops: [0, 100]
      //   }
      // }
    },
    zoomUsers: [],
    filterDate: [],
    expand: [],
    expanded: [],
    isExpanded: [],
    participations: [],
    zoomModal: false,
    singleExpand: true,
    isLoading: false,
    fullPage: true,
    groupId: null,
    userId: null,
    selectedRow: null,
    // userZoomId: null,
    mainHeaders: [
      { text: "", value: "avatar" },
      { text: "Name", value: "display_name" },
      { text: "Email Address", value: "email_address" },
      { text: "Group", value: "g.name" },
      { text: "Latest Activity", value: "latest_activity" },
      { text: "Lessons Complete", value: "lessons_completed" },
      { text: "Last Login", value: "last_login" },
      // { text: "Zoom Stats", value: "zoom_stats" }
    ],
    courseActivity: [
      { text: "Course Activity", value: "activity_title" },
      { text: "Date Started", value: "activity_started" },
      { text: "Date Completed", value: "activity_completed" },
      { text: "Duration", value: "activity_duration" },
    ],
    lessonActivity: [
      { text: "Lesson Activity", value: "activity_title" },
      { text: "Related Course", value: "course_title" },
      { text: "Date Started", value: "activity_started" },
      { text: "Date Completed", value: "activity_completed" },
      { text: "Duration", value: "activity_duration" },
    ],
    topicActivity: [
      { text: "Topic Activity", value: "activity_title" },
      { text: "Related Course", value: "course_title" },
      { text: "Date Started", value: "activity_started" },
      { text: "Date Completed", value: "activity_completed" },
      { text: "Duration", value: "activity_duration" },
    ],
    quizActivity: [
      { text: "Quiz Activity", value: "activity_title" },
      { text: "Related Course", value: "course_title" },
      { text: "Date Started", value: "activity_started" },
      { text: "Date Completed", value: "activity_completed" },
      { text: "Duration", value: "activity_duration" },
    ],
    courseHeaders: [
      { text: "Course", value: "course_title" },
      { text: "Status", value: "course_status" },
      { text: "Progress", value: "course_progress" },
      { text: "Date Started", value: "date_started", type: "date" },
      { text: "Date Completed", value: "date_completed", type: "date" },
    ],
    quizHeaders: [
      { text: "Course / Quiz", value: "quiz_title" },
      { text: "Passed", value: "pass" },
      { text: "Score", value: "score" },
      { text: "Duration", value: "timespent" },
      { text: "Date Started", value: "quiz_started", type: "date" },
      { text: "Date Completed", value: "quiz_completed", type: "date" },
    ],
    zoomHeaders: [
      { text: "Date", value: "date" },
      { text: "Joined", value: "join_date" },
      { text: "Parted", value: "leave_date" },
      { text: "Duration", value: "duration" },
    ],
    sortByGroup: "name",
    sortDescGroup: false,
    sortByCourse: "course_title",
    sortDescCourse: false,
  }),
  filters: {
    duration: function (value) {
      return value === null || value == 0
        ? "––"
        : moment.utc(value * 1000).format("HH:mm:ss");
      // return value;
    },
    status: function (value) {
      if (value == "not-started") {
        return "Not Started";
      } else if (value == "in-progress") {
        return "In Progress";
      } else if (value == "completed") {
        return "Completed";
      } else {
        return value;
      }
    },
    upperCaseNames: function (name) {
      return name.charAt(0).toUpperCase() + name.slice(1);
    },
    passed: function (value) {
      if (value == 1) {
        return "Passed";
      } else {
        return "Not Passed";
      }
    },
    fancyTimeFormat: function (duration) {
      let hrs = ~~(duration / 3600);
      let mins = ~~((duration % 3600) / 60);
      let secs = ~~duration % 60;
      let ret = "";

      if (hrs > 0) {
        ret += "" + hrs + ":" + (mins < 10 ? "0" : "");
      }

      ret += "" + mins + ":" + (secs < 10 ? "0" : "");
      ret += "" + secs;
      return ret;
    },
  },
  computed: {
    selectedGroups() {
      return this.$store.state.filters.selectedGroup;
    },

    // multipleGroupUsers() {
    //   return this.$store.state.multipleGroupUsers;
    // },

    randomData: function () {
      return Math.floor(Math.random() * 40 + 40);
    },
    // filteredData: function() {
    //     let a = this,
    //         b = a.filterDate ? a.filterDate[0] : [],
    //         c = new Date(a.$moment(b).format("YYYY-MM-DD")),
    //         d = a.filterDate ? a.filterDate[1] : [],
    //         e = new Date(a.$moment(d).format("YYYY-MM-DD"));
    //     return _.filter(a.$root.$data.documents, function(f) {
    //         if (_.isEmpty(a.filterDate) || (_.isNull(b) && _.isNull(d))) return !0;
    //         var g = new Date(a.$moment(f.created_at).format("YYYY-MM-DD"));
    //         return g >= c && g <= e;
    //     });
    //   },
    // },

    ...mapGetters({
      userDetails: "userDetails",
    }),
    ...mapGetters({
      filteredQuizzes: "filteredQuizzes",
    }),
    ...mapGetters({
      filteredCourses: "filteredCourses",
    }),
    ...mapGetters({
      multipleGroupUsers: "multipleGroupUsers",
    }),

    ...mapGetters({
      uniqueUsers: "uniqueUsers",
    }),

    ...mapGetters({
      userActivity: "userActivity",
    }),

    // ...mapGetters(["userCoursesById"]),
    // userCourses() {
    //   console.log(
    //     "userCourses:",
    //     this.$store.getters["userCoursesById"](this.userId)
    //   );
    //   return this.$store.getters["userCoursesById"](this.userId);
    //   // return this.userCoursesById(this.userId).userCourses
    // },
    // userCourseLessons: function() {
    //     return this.$store.getters["courseLessons"];
    // },
  },
  methods: {
    // loadUserDetails({ item }) {
    //   this.userId = item.id;
    //   // Bus.$emit("loadUserDetails", item.id);
    //   this.$refs.userDetails.getUserCoursesById(this.userId);
    // },
    getColor(percentage) {
      if (percentage <= 30) return "#555";
      else if (percentage <= 75) return "orange";
      else return "green";
    },
    completionPercentage(completed, total) {
      return ((completed / total) * 100).toFixed(2);
    },
    rowSelect(index) {
      this.selectedRow = index;
    },

    async getGroupUsers(groupId) {
      try {
        this.isLoading = true;
        // const res = await this.$axios.get('v2/groups/' + this.$route.params.id + '/users')
        const res = await this.$axios.get("ldd/v2/group-users/" + groupId);
        console.log("Group Users", res.data);
        this.$store.dispatch("getGroupUsers", res.data);
        // console.log(res.data)
      } catch (e) {
        this.error(e.response ? e.response.data.message : e.message);
      } finally {
        this.isLoading = false;
      }
    },

    // randomNumber: function() {
    //   return Math.floor(Math.random() * 30 + 10);
    // },
    // randomDate: function randomDate(start, end, startHour, endHour) {
    //   var date = new Date(+start + Math.random() * (end - start));
    //   var hour = (startHour + Math.random() * (endHour - startHour)) | 0;
    //   date.setHours(hour);
    //   return date.toDateString();
    // },
    // async getUserCoursesById(userId) {
    //   try {
    //     this.isLoading = true; /// doesn't load here when triggering the vue-data-table opening
    //     const res = await this.$axios.get("ldlms/v2/users/" + userId + "/courses");
    //     this.$store.dispatch("getUserCourses", res.data);
    //   } catch (e) {
    //     this.error(e.response ? e.response.data.message : e.message);
    //   } finally {
    //     this.isLoading = false;
    //   }
    // },
    // async getUserCourseProgress(userId) {
    //   try {
    //     this.userProgress = [];
    //     this.isLoading = true;
    //     const res = await this.$axios.get("ldlms/v2/users/" + userId + "/course-progress");
    //     this.$store.dispatch("getUserCourseProgress", res.data);
    //     console.log("Course Progress:", res.data);
    //     // this.userProgress = res.data;
    //   } catch (e) {
    //     this.error(e.response ? e.response.data.message : e.message);
    //   } finally {
    //     this.isLoading = false;
    //   }
    // },
    // async getUserQuizProgress(userId) {
    //   try {
    //     this.userProgress = [];
    //     this.isLoading = true;
    //     const res = await this.$axios.get("ldlms/v2/users/" + userId + "/quiz-progress");
    //     this.$store.dispatch("getUserQuizProgress", res.data);
    //     console.log("Quiz Progress:", res.data);
    //     // this.userProgress = res.data;
    //   } catch (e) {
    //     this.error(e.response ? e.response.data.message : e.message);
    //   } finally {
    //     this.isLoading = false;
    //   }
    // },

    async getUserDetails(userId) {
      try {
        // this.userDetails = [];
        this.isLoading = true;
        const res = await this.$axios.get("ldd/v2/users/" + userId);
        this.$store.dispatch("getUserDetails", res.data);
        console.log("User Details:", res.data);
        // this.userProgress = res.data;
      } catch (e) {
        this.error(e.response ? e.response.data.message : e.message);
      } finally {
        this.isLoading = false;
      }
    },

    async getSubscriber(subscriberId) {
      try {
        this.isLoading = true;
        const subscriberId = 487346653;
        const apiKey = "8xjZlAPwIpU62U7SQjjS-Q";
        const apiSecret = "1cddU-Wg7MSePN3JYZqa3G4pGs13I9fQfs1aLsEINbg";
        const tags = axios.get(
          `https://api.convertkit.com/v3/subscribers/${subscriberId}/tags?api_key=${apiKey}`
        );
      } catch (e) {
        this.error(e.request ? e.request.statusText : e.request);
      } finally {
        this.isLoading = false;
      }
    },

    /*async getUserZoomId(email) {
      try {
        // this.userDetails = [];
        this.isLoading = true;
        const res = await this.$axios.get(
          "ldd/v2/zoom-users/?email=" + email
        );
        // this.$store.dispatch("getUserDetails", res.data);
        const zoomId = res.data.id;
        
        if (zoomId === undefined) {
          throw new Error("No email associated with this account");
        }
        
        this.userZoomId = zoomId;

        // this.userProgress = res.data;
      } catch (e) {
        this.error(e.response ? e.response.data.message : e.message);
      } finally {
        this.isLoading = false;
      }
    },*/

    async openUserStats(email) {
      try {
        this.isLoading = true;

        const res = await this.$axios.get(
          "ldd/v2/meetings/meeting-history/?email=" + email
        );

        console.log("Zoom Data: ", res);

        this.participations = [
          ...res.data.map((participation) => ({
            join_date: new Date(participation.join_time).toLocaleString(),
            leave_date: new Date(participation.leave_time).toLocaleString(),
            duration: `${Math.floor(participation.duration / 60)} mins`,
            date: new Date(participation.join_time).toDateString(),
          })),
        ];

        console.log("Participation: ", this.participations);

        this.zoomModal = true;
      } catch (e) {
        this.error(e.response ? e.response.data.message : e.message);
        // this.zoomModal = false;
      } finally {
        this.isLoading = false;
      }
    },
    error(msg) {
      this.$store.commit("SET_MESSAGE", msg);
    },

    selectUser: function (userId) {
      this.userId = userId;
      // Bus.$emit("selectUser", userId);
      // this.getUserCoursesById(userId);
      // this.getUserCourseProgress(userId);
      // this.getUserQuizProgress(userId);

      this.getUserDetails(userId);
      this.$store.dispatch("getUserId", userId);
      this.$store.dispatch("getUserActivity");
    },
    // prepareData(updatedCourses) {
    //   for (const course of updatedCourses.entries()) {
    //     console.log(course)
    //     this.getUserCourseLessons(course.id);
    //   }
    // }
  },
  created() {
    Bus.$on("selectGroup", (selectedGroup) => {
      this.groupId = selectedGroup;
      this.getGroupUsers(selectedGroup);
    });

    this.getSubscriber();
    // this.getSubscribersAll();
  },
  watch: {
    // userCourses: function(updatedCourses) {
    //   console.log('userCourses Changed:', updatedCourses)
    //   this.prepareData(updatedCourses);
    // }
  },
};
</script>

<style lang="scss">
$red: #cb080f;
$black: #000;
$white: #fff;
$grey: #707070;

@import "~vuetify/src/styles/settings/_variables.scss";

body {
  background-color: $black;
}

thead {
  th {
    background-color: $black;
    vertical-align: middle;

    font-size: 14px;
    font-weight: 700;
    text-transform: uppercase !important;
  }
}

.section {
  &__heading {
    display: flex;
    align-items: center;
    width: 100%;
    height: 80px;
    font-size: 24px;
    font-weight: 800;
  }
}

.header {
  &__title {
    width: 600px;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
  }
}

.data__row {
  font-size: 16px;
  font-weight: 700;
}

.data__column {
  @media #{map-get($display-breakpoints, 'sm-and-down')} {
  }

  &.avatar {
    @media #{map-get($display-breakpoints, 'sm-and-down')} {
      height: auto !important;
      padding-top: 10px !important;
      padding-bottom: 10px !important;
    }
  }
}

.data {
  &__row {
    height: 100px;
    background-color: $black;

    @media #{map-get($display-breakpoints, 'sm-and-down')} {
      height: auto !important;
      padding-bottom: 10px !important;
    }

    &:nth-of-type(odd) {
      border-bottom: 1px solid $grey;
      background: #000
        linear-gradient(0deg, rgb(0, 0, 0, 1) 0%, rgb(112, 112, 112, 0.3) 100%);
    }
  }

  &__column {
    vertical-align: middle;
  }
}

.theme--light {
  span {
    color: #fff !important;
  }

  td {
    color: #fff !important;
  }

  i.v-icon {
    color: #000 !important;
  }

  .data {
    &__column {
      color: #fff !important;
    }
  }

  .header {
    &__label {
      color: #000 !important;
    }
  }

  .modal-button {
    color: #000 !important;
    text-transform: capitalize;
    z-index: 1;
  }

  .v-list-item--active {
    background-color: $red;

    .v-icon {
      color: #fff !important;
    }
  }

  .v-label--active.primary--text {
    color: $black !important;
  }

  .v-application .primary--text {
    color: $black !important;
  }

  .v-data-table
    > .v-data-table__wrapper
    > table
    > tbody
    > tr:hover:not(.v-data-table__expanded__content):not(.v-data-table__empty-wrapper) {
    background: #333 !important;
    transition: background 0.5s ease-in-out;
    cursor: pointer;
  }

  .v-data-table__wrapper {
    i.v-icon {
      color: #fff !important;
    }
  }
}

.user-details {
  &__wrapper {
    background-color: $red !important;
  }

  &__table {
    background-color: $red !important;
  }

  &__container {
    background-color: $red !important;
    border-radius: 0;
    padding-top: 20px !important;
    padding-bottom: 20px !important;

    @media #{map-get($display-breakpoints, 'md-and-up')} {
      padding: 20px 40px !important;
    }

    @media #{map-get($display-breakpoints, 'sm-and-down')} {
      padding: 0 !important;
    }

    thead {
      th {
        position: sticky !important;
        top: 0 !important;
        background-color: $red !important;
        font-size: 14px;
        font-weight: 700;
        text-transform: uppercase !important;
        z-index: 2;
      }
    }
  }
}

.progress-chart {
  &__container {
    font-weight: 700;
    color: $black;
    background: $red;
    border-radius: 10px;
  }

  &__title {
    font-size: 20px;
    font-weight: 600;
    text-align: left;
    color: #fff;
    padding-left: 20px;
  }
}

.custom-highlight-row {
  background: none !important;
  background-color: $red !important;
  border-bottom: none !important;
}

.theme--dark.v-application {
  background: $black !important;
  color: #fff !important;
}

.theme--dark.v-data-table
  > .v-data-table__wrapper
  > table
  > tbody
  > tr:hover:not(.v-data-table__expanded__content):not(.v-data-table__empty-wrapper) {
  background: #777 !important;
  transition: background 0.5s ease-in-out;
  cursor: pointer;
}

// .theme--dark.v-data-table>.v-data-table__wrapper>table>tbody>tr:not(:last-child)>td:last-child, .theme--dark.v-data-table>.v-data-table__wrapper>table>tbody>tr:not(:last-child)>td:not(.v-data-table__mobile-row), .theme--dark.v-data-table>.v-data-table__wrapper>table>tbody>tr:not(:last-child)>th:last-child, .theme--dark.v-data-table>.v-data-table__wrapper>table>tbody>tr:not(:last-child)>th:not(.v-data-table__mobile-row), .theme--dark.v-data-table>.v-data-table__wrapper>table>thead>tr:last-child>th {
//   border-bottom: none !important;
// }

// .v-data-table>.v-data-table__wrapper>table>tbody>tr>td {
//   height: auto !important;
// }

.v-data-table__wrapper {
  overflow: unset !important;
}

.v-data-table
  > .v-data-table__wrapper
  tbody
  tr:first-child:hover
  td:first-child {
  border-top-left-radius: 0 !important;
}

.v-data-table
  > .v-data-table__wrapper
  tbody
  tr:first-child:hover
  td:last-child {
  border-top-right-radius: 0 !important;
}

.v-data-table--fixed-header > .v-data-table__wrapper > table > thead > tr > th {
  background-color: $red !important;
}

@media #{map-get($display-breakpoints, 'sm-and-down')} {
  .theme--dark.v-data-table
    > .v-data-table__wrapper
    > table
    > tbody
    > tr:not(:last-child)
    > td:not(.v-data-table__mobile-row) {
    border-bottom: 0 !important;
  }
}

.v-text-field.v-text-field--solo .v-input__control {
  min-height: 10px;
}

.v-text-field.v-text-field--enclosed .v-text-field__details {
  display: none !important;
}

.v-input__slot {
  margin-bottom: 0;
}

::-webkit-scrollbar {
  width: 15px;
}

::-webkit-scrollbar-track {
  background: #202020;
  border-left: 1px solid #2c2c2c;
}

::-webkit-scrollbar-thumb {
  background: #3e3e3e;
  border-radius: 7px;
}

::-webkit-scrollbar-thumb:hover {
  background: white;
}

.user-activity__inner-container::-webkit-scrollbar,
.user-details__inner-container::-webkit-scrollbar {
  width: 15px !important;
  margin-left: 20px;
}

.user-activity__inner-container::-webkit-scrollbar-track,
.user-details__inner-container::-webkit-scrollbar-track {
  background: rgba(255, 255, 255, 0.3);
  border-left: none !important;
  border-radius: 10px !important;
}

.user-activity__inner-container::-webkit-scrollbar-thumb,
.user-details__inner-container::-webkit-scrollbar-thumb {
  background: #fff !important;
  border: solid 3px #fff !important;
  border-radius: 7px !important;
}

.user-activity__inner-container::-webkit-scrollbar-thumb:hover,
.user-details__inner-container::-webkit-scrollbar-thumb:hover {
  background: white !important;
}

.user-details__container {
  .theme--dark.v-data-table
    > .v-data-table__wrapper
    > table
    > tbody
    > tr:hover:not(.v-data-table__expanded__content):not(.v-data-table__empty-wrapper) {
    background: $red !important;
    transition: background 0.5s ease-in-out !important;
    cursor: pointer;
  }
}

.restrict-height {
  height: 500px;
  max-height: 100vh;
  overflow-y: auto;
  overflow-x: hidden;
}

.apexcharts-legend-text {
  color: #fff;
  font-weight: 700;
}

.apexcharts-xaxistooltip {
  color: #fff;
}

.zoom-statistics {
  position: relative;
  z-index: 1;
}

@media screen and (max-width: 768px) {
  .mobile table.v-table tr {
    max-width: 100%;
    position: relative;
    display: block;
  }

  .mobile table.v-table tr:nth-child(odd) {
    border-left: 6px solid deeppink;
  }

  .mobile table.v-table tr:nth-child(even) {
    border-left: 6px solid cyan;
  }

  .mobile table.v-table tr td {
    display: flex;
    width: 100%;
    border-bottom: 1px solid #f5f5f5;
    height: auto;
    padding: 10px;
  }

  .mobile table.v-table tr td ul li:before {
    content: attr(data-label);
    padding-right: 0.5em;
    text-align: left;
    display: block;
    color: #999;
  }

  .v-datatable__actions__select {
    width: 50%;
    margin: 0px;
    justify-content: flex-start;
  }

  .mobile .theme--light.v-table tbody tr:hover:not(.v-datatable__expand-row) {
    background: transparent;
  }
}
</style>
